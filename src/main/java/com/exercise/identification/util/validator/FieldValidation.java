package com.exercise.identification.util.validator;
...
public final class FieldValidation {
    ...
    public FieldValidation isEmail() {
        return isEmail(parameterMap.getException());
    }

    public FieldValidation isEmail(final RuntimeException exception) {
        return condition(value -> {
            boolean condition = false;
            if (Validator.isEmpty(value) || Validator.isNull(value)) {
                condition = false;
    ...
    public FieldValidation isDate() {
        return isDate(parameterMap.getException());
    }

    public FieldValidation isDate(final RuntimeException exception) {
        return condition(value -> {
            boolean condition = false;
            if (Validator.isEmpty(value) || Validator.isNull(value)) {
                condition = false;
            } else {
                condition = Validator.isDate(fieldValue);
            }
            return condition;
        }, exception);
    }
    ...
    public FieldValidation isGender() {
        return isGender(parameterMap.getException());
    }

    public FieldValidation isGender(final RuntimeException exception) {
        return condition(value -> {
            boolean condition = false;
            if (Validator.isEmpty(value) || Validator.isNull(value)) {
                condition = false;
            } else {
                condition = Validator.isGender(fieldValue);
            }
            return condition;
        }, exception);
    }
